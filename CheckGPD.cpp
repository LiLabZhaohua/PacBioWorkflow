...
int main (int argc, char **argv) {
    ...
    while(inf){
        ...
        if(strInput.length() > 0){
            ...
            if(gpd.vec_exon_start[0] > gpd.vec_exon_end[0] || gpd.vec_exon_end[gpd.nblock - 1] < gpd.vec_exon_start[gpd.nblock - 1])  DisplayGPD(gpd);
            //cout << gpd.vec_exon_end[gpd.nblock - 1] << '\t' << gpd.vec_exon_start[gpd.nblock - 1] << endl;
        }
    }
