...
int main (int argc, char **argv) {
    ...
    while(annoinf){
        ...
        int dist_up = 10000;
        int dist_down = 10000;
        if(three_prime_end.chro == pk_vec[locate].chro)  dist_up = abs(three_prime_end.pos - pk_vec[locate].pos);
        if(locate < pk_vec.size() - 1 && three_prime_end.chro == pk_vec[locate + 1].chro)  dist_down = abs(three_prime_end.pos - pk_vec[locate + 1].pos);
