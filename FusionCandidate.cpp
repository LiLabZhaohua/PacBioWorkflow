...
int main (int argc, char **argv) {
    ...
    cerr << "FusionCandidate <annotation.gpd> <reads.gpd>" << endl;
    ...
    while(gpdinf){
        ...
        if(strInput.length() > 0){
            ...
            int gene_second_max_overlap = 0;
            string second_max_gene_name = "*";
            for(it = gene_overlap_map.begin(); it != gene_overlap_map.end(); ++it){
                if((it->second) > gene_second_max_overlap){
                    gene_second_max_overlap = (it->second);
                    second_max_gene_name = (it->first);
                    (it->second) = -1;
                }
            }

            double ratio_max = gene_max_overlap / (gpd_len + 0.0);
            double ratio_second_max = gene_second_max_overlap / (gpd_len + 0.0);
            if(total_ratio > 0.8 && ratio_second_max > 0.1){
                cout << strInput << endl;
                cout << gpd.gname << '\t' << max_gene_name << '\t' << ratio_max << '\t' << second_max_gene_name << '\t' << ratio_second_max << endl;
            }
        }
    }
