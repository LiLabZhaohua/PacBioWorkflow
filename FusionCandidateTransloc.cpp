...
pair<string,int> MaxOverlapGene (GPD gpd, vector<EXON> &exon_vec) {
    ...
    double total_ratio = total_overlap_len / (gpd_len + 0.0);
    ...
    if(total_ratio > 0.8)  output_gene = max_gene_name;

    return  pair<string,int>(output_gene, total_overlap_len);
}       
...
int main (int argc, char **argv) {
    ...
    cerr << "FusionCandidateTransloc <annotation.gpd> <transloc_reads.gpd>" << endl;
    ...
    while(gpdinf){
        ...
        getline(gpdinf, strInput1);
        if(strInput1.length() == 0)  break;
        getline(gpdinf, strInput2);

        GPD gpd1, gpd2;
        BuildGPDFromString(strInput1, gpd1);
        BuildGPDFromString(strInput2, gpd2);

        pair<string,int> rslt1 = MaxOverlapGene(gpd1, exon_vec);
        pair<string,int> rslt2 = MaxOverlapGene(gpd2, exon_vec);
        string gene1 = rslt1.first;
        string gene2 = rslt2.first;
        double ratio1 = (rslt1.second) / (GPD_Length(gpd1) + GPD_Length(gpd2) + 0.0);
        double ratio2 = (rslt2.second) / (GPD_Length(gpd1) + GPD_Length(gpd2) + 0.0);

        if((gene1 != "*") && (gene2 != "*") && (gene1 != gene2))  cout << gpd1.gname << '\t' << gene1 << '\t' << ratio1 << '\t' << gene2 << '\t' << ratio2 << endl;
    }
