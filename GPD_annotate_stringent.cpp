...
pair<int,int> AlignGPD (GPD gpd_query, GPD gpd_base) {
    ...
    if(gpd_query.chro != gpd_base.chro || gpd_query.nblock != gpd_base.nblock || gpd_query.strand != gpd_base.strand)  score = 0;
        ...
        if(gpd_query.nblock == 1){
            ...
            if(gpd_query.strand == "+"){
                if(abs(gpd_query.start - gpd_base.start) <= 100)  left_aligned = true;
                if(abs(gpd_query.end - gpd_base.end) <= 30)  right_aligned = true;
            }
            else{
                if(abs(gpd_query.start - gpd_base.start) <= 30)  left_aligned = true;
                if(abs(gpd_query.end - gpd_base.end) <= 100)  right_aligned = true;
            }
            ...
            if(gpd_query.strand == "+"){
                if(abs(gpd_query.start - gpd_base.start) > 100 || abs(gpd_query.end - gpd_base.end) > 30)  aligned = false;
            }
            else{
                if(abs(gpd_query.start - gpd_base.start) > 30 || abs(gpd_query.end - gpd_base.end) > 100)  aligned = false;
            }
            divergence = divergence + abs(gpd_query.start - gpd_base.start) + abs(gpd_query.end - gpd_base.end);
...
int main (int argc, char **argv) {
    ...
    cerr << "GPD_annotate_stringent <annotation.gpd> <read_annotate.gpd>" << endl;
    ...
    while(rinf){
        ...
        if(strInput.length() > 0){
            ...
            if(vec_record.size() > 0){
                ...
                cout << gpd_query.gname << '\t' << vec_record[min_index].first.gname << '\t' << vec_record[min_index].first.tname << endl;
