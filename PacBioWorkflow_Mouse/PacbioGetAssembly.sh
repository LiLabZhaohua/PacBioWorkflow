...
then
  ...
  echo "*        6, *IntactIsoform_sorted.gpd                                                           *"
  echo "*        7, *IntactIsoform_filter.gpd (Filtered by Ends)                                        *"
  echo "*************************************************************************************************"
else
...
echo "  Final counts of the assembly: "${OutputPrefix}.IntactIsoform_sorted.gpd
...
echo "Step 06, Map reads to assembly, get Compatible reads, and antisense reads"
#MapReadToIsoform: This is a critical script to map LRs to the assembly GPD, and get intact and decay reads
MapReadToIsoform ${OutputPrefix}.IntactIsoform_sorted.gpd ${OutputPrefix}.LRs.ccs.declip.gpd | grep Compatible | awk '{print $1}' | sort -u | wc -l
